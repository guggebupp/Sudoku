        ...
        if (curl) {
        	void* ptr = curl.get();
                ...
                if (!basic_auth_pass.empty()) {
                        curl_easy_setopt(ptr, CURLOPT_HTTPAUTH,
                                         CURLAUTH_BASIC);
                        curl_easy_setopt(ptr, CURLOPT_USERPWD,
                                         basic_auth_pass.c_str());
                }

                // User agent
                curl_easy_setopt(ptr, CURLOPT_USERAGENT,
                                 Handle::user_agent.c_str());
                // Requested URL
                curl_easy_setopt(ptr, CURLOPT_URL, URI.c_str());
                switch (method) {
...
Handle::response Handle::execGet() {
        ...
        curl_easy_setopt((void*)curl.get(), CURLOPT_WRITEFUNCTION, write_callback);
        curl_easy_setopt((void*)curl.get(), CURLOPT_WRITEDATA, &res);
        ...
        if (!custom_headers.empty()) {
                ...
                curl_easy_setopt((void*)curl.get(), CURLOPT_HTTPHEADER, chunk);
        }

        // Header callback function
        curl_easy_setopt((void*)curl.get(), CURLOPT_HEADERFUNCTION, header_callback);
        curl_easy_setopt((void*)curl.get(), CURLOPT_HEADERDATA, &res);
        // Perform curl request
        const auto req = curl_easy_perform((void*)curl.get());
        if (req != CURLE_OK) {
        ...
        long http_code{0};
        curl_easy_getinfo((void*)curl.get(), CURLINFO_RESPONSE_CODE, &http_code);
...
Handle::response Handle::execPost() {
        ...
        curl_easy_setopt((void*)curl.get(), CURLOPT_POST, 1L);
        // Set post fields and post field size
        if (!post_content.empty()) {
                curl_easy_setopt((void*)curl.get(), CURLOPT_POSTFIELDS,
                                 post_content.c_str());
                curl_easy_setopt((void*)curl.get(), CURLOPT_POSTFIELDSIZE,
                                 post_content.size());
        } else {
                res.body =
...
Handle::response Handle::execPut() {
        ...
        curl_easy_setopt((void*)curl.get(), CURLOPT_PUT, 1L);
        curl_easy_setopt((void*)curl.get(), CURLOPT_UPLOAD, 1L);

        // set read callback function
        curl_easy_setopt((void*)curl.get(), CURLOPT_READFUNCTION, read_callback);
        // set data object to pass to callback function
        curl_easy_setopt((void*)curl.get(), CURLOPT_READDATA, &up_obj);
        // Write callback function
        curl_easy_setopt((void*)curl.get(), CURLOPT_WRITEFUNCTION, write_callback);
        curl_easy_setopt((void*)curl.get(), CURLOPT_WRITEDATA, &res);
        ...
        curl_easy_setopt((void*)curl.get(), CURLOPT_HEADERDATA, &res);
        // set data size
        curl_easy_setopt((void*)curl.get(), CURLOPT_INFILESIZE,
...
Handle::response Handle::execDel() {
        ...
        curl_easy_setopt((void*)curl.get(), CURLOPT_CUSTOMREQUEST, "DELETE");
        // Write callback function
        curl_easy_setopt((void*)curl.get(), CURLOPT_WRITEFUNCTION, write_callback);
        curl_easy_setopt((void*)curl.get(), CURLOPT_WRITEDATA, &res);
        struct curl_slist *chunk = nullptr;
